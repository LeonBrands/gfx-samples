// based off 001_triangle
// uses a vertex buffer to draw a quad, instead of hardcoded vertices inside of the shader
// The differences can be found at:
// * Line 536-556: Vertex input bindings/attributes
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 800, "002_vertex_buffer", nullptr, nullptr);
    ...
    appInfo.pApplicationName = "002_vertex_buffer";
    ...
    THROW_IF_FAILED(glfwCreateWindowSurface(instance, window, nullptr, &surface));
    ...
    THROW_IF_FAILED(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, surface, &surfaceCapabilities));
    ...
    THROW_IF_FAILED(vkCreateSwapchainKHR(device, &swapchainInfo, nullptr, &swapchain));
    ...
    for (size_t i = 0; i < swapchainImages.size(); i++)
    {
        ...
        THROW_IF_FAILED(vkCreateImageView(device, &viewInfo, nullptr, &swapchainImageViews[i]));
    ...
    std::ifstream vertexFile("../src/002_vertex_buffer/vertex.spv", std::ios::ate | std::ios::binary);
    ...
    THROW_IF_FAILED(vkCreateShaderModule(device, &vertexShaderInfo, nullptr, &vertexShader));
    
    // the same process is done for our fragment shader
    std::ifstream fragmentFile("../src/002_vertex_buffer/fragment.spv", std::ios::ate | std::ios::binary);
    ...
    THROW_IF_FAILED(vkCreateShaderModule(device, &fragmentShaderInfo, nullptr, &fragmentShader));
    ...
    // Describe our vertex buffer's size and usage
    ...
    if (presentFamily != graphicsFamily)
    {
        ...
        bufferInfo.queueFamilyIndexCount = families.size();
    ...
    THROW_IF_FAILED(vkCreateBuffer(device, &bufferInfo, nullptr, &vertexBuffer));
    ...
    // Before we start allocating memory however, we should first query the physical device's memory properties.
    ...
    for (size_t i = 0; i < memoryProperties.memoryTypeCount; i++)
    {
        ...
        // for simplicity's sake we'll just use a host_coherent buffer for now
    ...
    vertexAllocInfo.pNext = nullptr;
    vertexAllocInfo.allocationSize = memoryReqs.size;
    vertexAllocInfo.memoryTypeIndex = index;
    
    VkDeviceMemory vertexBufferMemory;
    THROW_IF_FAILED(vkAllocateMemory(device, &vertexAllocInfo, nullptr, &vertexBufferMemory));

    // finally, bind the vertex buffer and its memory
    THROW_IF_FAILED(vkBindBufferMemory(device, vertexBuffer, vertexBufferMemory, 0));
    
    // copy vertex data to our vertex buffer
    void* ptr;
    THROW_IF_FAILED(vkMapMemory(device, vertexBufferMemory, 0, sizeof(float) * vertices.size(), 0, &ptr));
    memcpy(ptr, vertices.data(), sizeof(float) * vertices.size());
